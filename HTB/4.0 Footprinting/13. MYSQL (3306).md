...
#### IMP::
MySQL server does not use an empty password for the user `root`, but a fixed password.
```shell
mysql -u root -h 10.129.14.132
mysql -u root -pP4SSw0rd -h 10.129.14.128 => no space in -p and password
```
![](../../Attachements/Pasted%20image%2020260629103512.png)

```shell
mysql -u root -pP4SSw0rd -h 10.129.14.128 =>after login use following commands
show databases;
select version();
use database1;
show tables;
...
This commands sometime eork sometime wont:
![](../../Attachements/Pasted%20image%2020260629110306.png)

```shell
mysql -h 10.129.67.134 -u robin -probin  --ssl-verify-server-cert=FALSE 
```

select name, email from myTable where name = "Otto Lang";