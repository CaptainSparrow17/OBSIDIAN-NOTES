...
Uh, also it says Microsoft intra roles can be assigned. So this gets into giving out roles and permissions and all that which I'm not doing in this video. Then I've got membership type assigned so I could make it a manual type of group and assign group where you select and specify your members. If you want, uh, manually. Or you can do a dynamic group where it can be done dynamically. Okay. Which is what I'm going to go with. Sensitivity label. You may not have that in your environment. I've implemented sensitivity labels and that's outside the scope of what we're learning right now in this video. Okay. But um, that gets into sensitivity and compliance and all that stuff. But don't worry about that if you don't see that in your own. If you're doing this on your own right now, don't worry about it if you don't see that, okay. But let's talk about what a dynamic user is, okay. So I can do a sign that's manual or dynamic user. So if I do dynamic user um let me specify group owner first. So I will do that and I'll just say JK I'll be the group owner. So John Christopher right here select. But here is where you're going to set your dynamic rule. So a dynamic query. So a dynamic query is going to be what decides who's a part of this group. Okay. So if I go and I click add dynamic query and dynamic queries are going to be based on rules that that are based on what we call expressions. This expression is going to contain some properties. And then what is called an operator that decides whether or not you are part of this. Okay. So let me show you an example. So if we go to property, look these are all the properties that you can have with users. Let's go with department and let's say equals and we'll put sales okay. So department equals sales. And you don't have to worry about case sensitivity and all that. It's not case sensitive. So go ahead and add us. That'll be our expression. I'll click this white area here. And then you should see the save button up here. Also you'll see the